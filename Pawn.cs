...
namespace Polish_Draughts
{
	public class Pawn
	{
		...
		{
            int oppositeMark = 2;
            if (row < 0 || col < 0 || row > board.size - 1 || col > board.size - 1) { return false; }
            if (board.field[row,col] != null) { return false; }
            ...
            {
                if (Coordinates[0] - row == 2 && board.field[Coordinates[0] - 1, Coordinates[1] + 1] == oppositeMark)
                {
                    return true;
                } else if (Coordinates[0] - row == - 2 && board.field[Coordinates[0] + 1, Coordinates[1] + 1] == oppositeMark) 
                { 
                    return true; 
                }
        ...
        {   if (CanMoveTo(Coordinates[0] - 1, Coordinates[1] - 1, board)) { return true; }
            if (CanMoveTo(Coordinates[0] - 2, Coordinates[1] - 2, board)) { return true; }
            if (CanMoveTo(Coordinates[0] + 1, Coordinates[1] - 1, board)) { return true; }
            if (CanMoveTo(Coordinates[0] + 2, Coordinates[1] - 2, board)) { return true; }
            if (CanMoveTo(Coordinates[0] - 1, Coordinates[1] + 1, board)) { return true; }
            if (CanMoveTo(Coordinates[0] - 2, Coordinates[1] + 2, board)) { return true; }
